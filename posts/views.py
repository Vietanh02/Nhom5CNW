...
client = razorpay.Client(auth=("1", "1"))
...
def post_details(request, category_slug, slug):
    ...
    if request.method == "POST" and request.user.is_authenticated:
        ...
        review = Reviews.objects.create(
            post=allpost, user=request.user, stars=allstars, content=allcontent
        )
...
def edit_profile(request):
    if request.method == "POST":
        user_form = CustomerCreationEditForm(
            request.POST or None, request.FILES or None, instance=request.user
        )
        profile_form = CustomerEditForm(
            request.POST or None, request.FILES or None, instance=request.user.customer
        )
...
def add_to_cart(request, slug):
    ...
    order_item = Cart.objects.get_or_create(
        item=course, user=request.user, purchase=False
    )
...
def cart_view(request):
    ...
    if carts.exists() and orders.exists():
        ...
        return render(
            request,
            "core/cart.html",
            context={"carts": carts, "order": order, "orders": orders},
        )
    else:
...
def approve_certificates(request, id):
    if request.method == "POST":
        ...
        approve_cert = approve_certForm(
            request.POST or None, request.FILES or None, instance=carts
        )
...
def edit_post(request, id):
    if request.method == "POST":
        ...
        editpostForm = EditPostForm(
            request.POST or None, request.FILES or None, instance=posts
        )
...
def edit_course(request, id):
    if request.method == "POST":
        ...
        editcourse = EditMaincourse(
            request.POST or None, request.FILES or None, instance=course
        )
...
def usercertificate(request, category_slug, slug):
    ...
    if carts.exists() and orders.exists():
        ...
        return render_to_pdf(
            "users/certificate.html",
            {
                "customerName": request.user.first_name,
                "customerNamelast": request.user.last_name,
...
def edit_videos(request, id):
    if request.method == "POST":
        ...
        editvideoForm = videoform(
            request.POST or None, request.FILES or None, instance=vid
        )
...
def edit_blank(request, id):
    if request.method == "POST":
        ...
        editblank = blankform(
            request.POST or None, request.FILES or None, instance=blank
        )
...
def edit_leftcat(request, id):
    if request.method == "POST":
        cat = Category.objects.get(id=id)
        editcatForm = leftmenu(
            request.POST or None, request.FILES or None, instance=cat
        )
        if editcatForm.is_valid():
            editcatForm.save()
            messages.success(request, "Category Update Sucessfully !!")
...
def edit_admin_review(request, id):
    ...
    return render(
        request, "webadmin/edit_admin_reviews.html", {"edit": edit_admin_reviews}
    )
...
def paypal_check(request):
    ...
    order_payment.emailAddress = user.email
    order_payment.coupon = order.coupon
    order_payment.order_id = order.order_id
    order_payment.total = order.get_totals()
    order_payment.save()
...
def paypal_return(request):
    messages.info(request, "Success Payment")
    return render(request, "core/checkout.html")


def payal_cancel(request):
    messages.info(request, "Error Payment")
    return render(request, "core/checkout.html")
